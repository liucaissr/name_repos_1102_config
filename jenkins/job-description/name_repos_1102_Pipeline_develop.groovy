...
def customLib = library identifier: 'name_repos_1102_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/ORGANISATION/name_repos_1102_lib.git',
   credentialsId: 'git-token-credentials']) 
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "name_repos_1102_Pipeline_develop", "feature-1668775521557")
                }
            }    
        }
        stage('Build') {
